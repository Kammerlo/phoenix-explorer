...
- [`config/cache.ts`](packages/gateway/src/config/cache.ts): `NodeCache` behind a single generic `cachedFetch(key, ttl, fetcher, onError?)` — cache hit → **in-flight dedupe** (N concurrent identical lookups trigger exactly one upstream call) → fetch + store; `onError` lets a helper degrade to a fallback value with its own TTL. Helpers: `getEpoch`, `getBlock`, `getLatestBlock` (15 s tip TTL), `getBlockTxs` (block→tx-hash list), `getTransactions`, `getTxMetadata`, `getUtxos`, `getTxDetail` / `setTxDetail`, `fetchAddressTotal`, `getAsset` (asset metadata, 1 h), and the 24 h immutable-by-hash helpers `getTxRedeemers` / `getScriptCbor` / `getDatumCbor`. **When adding any per-item Blockfrost lookup, add/use a `cachedFetch`-backed helper — never call `API.*` in a loop.** See [Transaction Detail Performance](#transaction-detail-performance).
