...
- **Yaci connector:** [`yaci/mapper/ToContracts.ts`](packages/frontend/src/commons/connector/yaci/mapper/ToContracts.ts) derives one `SPEND` contract per datum-bearing (script-locked) input (yaci-store exposes inline datums / reference scripts on UTxOs but no per-redeemer ExUnits, so those are 0). `TxUtxoToUtxo` now keeps the datum/script fields.
